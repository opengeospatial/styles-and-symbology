...
Variables are encoded as JSON Pointer to a top-level `"$variables"` section in the same source file, or in another included file.
...
or an array of relative file path strings to include.
