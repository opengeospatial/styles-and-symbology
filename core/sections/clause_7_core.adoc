...
Conceptually, all Symbolizer properties can be specified using a _ParameterValue_, but this is not required by
this core conformance class, and specific requirements classes such as the "Symbolizer Parameter Value Expresssions"
requirements class enable this behavior.
...
* A `null` literal represents represents an unset value
...
Implementations are also required to support prioritizing operations (typically parentheses `(` `)`).
...
An array expressions defines zero, or more element expressions forming an ordered set.
