...
| *Styling Rules*   | 
| name              | `<UserStyle>` with <Name>
| selector          | `<Rule>` with `<ogc:Filter>`
...
| visualization.dateTime         | `<ogc:PropertyName>` with datetime (timestamp) value  and `<ogc:Literal>`
| visualization.date             | `<ogc:PropertyName>` with date value and `<ogc:Literal>`
| visualization.timeOfDay        | `<ogc:PropertyName>` with time value and `<ogc:Literal>`
| visualization.pass             | N/A (drawing order)
